...
class _DashboardFullScreenState extends State<DashboardFullScreen> {
    ...
    return BlocBuilder<DashCubit, DashStates>(
      builder: (context, state) {
        ...
        if (state is GetDashboardOverviewSuccess)
          overview = state.postgresDashboardOverview;
        if (state is GetDashboardMetricsSuccess)
          metrics = state.postgresDashboardMetrics;
                  ...
                  if (isMobile)
                    Column(
                      children: [
                        ...
                        const SizedBox(height: 24),
                        SidebarSction(metrics: metrics, overview: overview),
                    ...
                    Row(
                      ...
                      children: [
                             Expanded(
                          flex: 1,
                          child: ProjectAccessSection(
                          
                          ),
                        ),
                        Expanded(
                          flex: 2,
                          child: QuerySection(queries: queries),
                        ),
  ...
  List<StatCard> _buildTopCards(dynamic overview) => [
    StatCard(
      title: 'Total Tables',
      value: '${overview?.schemaSummary?.totalTables ?? 0}',
    ...
    StatCard(
      title: 'Total Columns',
      value: '${overview?.schemaSummary?.totalColumns ?? 0}',
    ...
    StatCard(
      title: 'Primary Keys',
      value: '${overview?.schemaSummary?.totalPrimaryKeys ?? 0}',
    ...
    StatCard(
      ...
      value: overview?.db?.database ?? 'N/A',
  ...
  List<StatCard> _buildBottomCards(dynamic metrics) => [
    ...
    StatCard(
      ...
      value: '${metrics?.blockedSessions ?? 0}',
